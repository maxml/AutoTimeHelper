...
import com.maxml.timer.entity.CallEvent;
...
public class CallEventCRUD {
    ...
    public CallEventCRUD(Handler handler) {
    ...
    public void create(CallEvent callEvent) {
        Log.i("Slice", " Slice starting create");

        // get Firebase id
        String dbId = userRef.push().getKey();
        callEvent.setId(dbId);

        // create update map
        Map<String, Object> data = new HashMap<>();

        // put data for sort by day
        long dayCount = Utils.getDayCount(callEvent.getStartDate());
        if (dayCount == -1) {
            handler.sendEmptyMessage(Constants.DB_RESULT_FALSE);
            return;
        }
        data.put("/" + Constants.SORT_BY_DATE_PATH + "/" + dayCount, dbId);

        // put data for call db
        data.put("/" + Constants.CALL_DATABASE_PATH , callEvent);
    ...
    public void read(String user) {
        if (user == null) return;
        userRef.orderByChild(Constants.COLUMN_USER_ID)
                .equalTo(user)
...
//
    ...
    public void update(Slice slice) {
        ...
        // set id entity
        slice.setId(key);
        userRef.child(key).setValue(slice).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    handler.sendEmptyMessage(Constants.DB_RESULT_OK);
...
//		});
    ...
    public void delete(String id) {
        userRef.child(id).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
