...
public class GoogleMapLogic extends ActionBarActivity implements OnMapReadyCallback, OnLocationChangedListener {
	...
	private void inRadius() {
		...
		Slice slice = new Slice(ParseUser.getCurrentUser().getObjectId(), line, starttime, finishtime, "walk time",
				SliceType.WALK);
	...
	private void outTheRadius() {
		...
		Point start = new Point(pointRadius.getLat(), pointRadius.getLong());
		Line line = new Line(start, finish, ParseUser.getCurrentUser().getObjectId());
		Slice slice = new Slice(ParseUser.getCurrentUser().getObjectId(), line, starttime, finishtime, "rest time",
				SliceType.REST);
		controllerGoogleMap.addSlise(slice);
		Log.d(LOG, "start = " + start.toString());
		Log.d(LOG, "finish = " + finish.toString());
